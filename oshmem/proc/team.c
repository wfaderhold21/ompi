...
static int oshmem_team_create_shared(void)
{
    ...
    /* Sync/work for TEAM_SHARED allocated later in oshmem_team_alloc_predefined_sync_buffers() */
    _oshmem_team_shared.sync_size = OSHMEM_TEAM_SYNC_SIZE;
    _oshmem_team_shared.work_size = OSHMEM_TEAM_WORK_SIZE;
    _oshmem_team_shared.sync = NULL;
    _oshmem_team_shared.work = NULL;
...
int oshmem_team_init(void)
{
     ...
     * Sync/work buffers are allocated later in oshmem_team_alloc_predefined_sync_buffers()
     * after the symmetric heap is ready (see oshmem_shmem_init). */
    ...
    _oshmem_team_world.sync_size = OSHMEM_TEAM_SYNC_SIZE;
    _oshmem_team_world.work_size = OSHMEM_TEAM_WORK_SIZE;
    _oshmem_team_world.sync = NULL;
    _oshmem_team_world.work = NULL;

    oshmem_team_world = &_oshmem_team_world;

    /* Initialize SHMEM_TEAM_SHARED (also defers sync buffer allocation) */
...
int oshmem_team_alloc_predefined_sync_buffers(void)
{
    int rc;

    if (oshmem_team_world != NULL && oshmem_team_world->sync == NULL) {
        rc = oshmem_team_alloc_sync_buffers(oshmem_team_world);
        if (OSHMEM_SUCCESS != rc) {
            return rc;
        }
    }
    if (oshmem_team_shared != NULL && oshmem_team_shared->sync == NULL) {
        rc = oshmem_team_alloc_sync_buffers(oshmem_team_shared);
        if (OSHMEM_SUCCESS != rc) {
...
int oshmem_team_sync(oshmem_team_t *team)
{
    ...
    /* Use the group's barrier collective with team's sync buffer */
