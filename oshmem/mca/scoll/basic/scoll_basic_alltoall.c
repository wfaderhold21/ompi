...
/* next up: update broadcast to use non-blocking puts */

int basic_alltoall_nb_progress(struct oshmem_group_t *group,
                             void *target,
                             const void *source,
                             ptrdiff_t dst, ptrdiff_t sst,
                             size_t nelems,
...
int basic_alltoall_nb_start(struct oshmem_group_t *group,
                             void *target,
                             const void *source,
                             ptrdiff_t dst, ptrdiff_t sst,
                             size_t nelems,
                             ...
                             void *coll)
{
    ...
    for (src_blk_idx = 0; src_blk_idx < group->proc_count; src_blk_idx++) {
        dst_pe = get_dst_pe(group, src_blk_idx, dst_blk_idx, &dst_pe_idx);
        rc = MCA_SPML_CALL(put_nb(oshmem_ctx_default, dst_blk,
                                nelems * element_size,
                                get_stride_elem(source, 1, nelems,
                                                element_size, dst_pe_idx, 0),
                             ...
                             shmem_req_h * request)
{
    ...
    module = (mca_scoll_basic_module_t *) group->g_scoll.scoll_alltoall_nb_module;
    int rc = OSHMEM_SUCCESS;
    ...
    coll->start = basic_alltoall_nb_start;
    coll->progress = basic_alltoall_nb_progress;
    ...
    coll->args.alltoall.dst = dst;
    coll->args.alltoall.sst = sst;
    coll->args.alltoall.element_size = element_size;
