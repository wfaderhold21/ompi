...
int mca_scoll_ucc_init_ctx(oshmem_group_t *osh_group)
{
    ...
    tm_requested = oshmem_mpi_thread_multiple ? UCC_THREAD_MULTIPLE : UCC_THREAD_SINGLE;
    ...
    cm->ucc_lib_attr.mask = UCC_LIB_ATTR_FIELD_THREAD_MODE | UCC_LIB_ATTR_FIELD_COLL_TYPES;
    ...
    if (UCC_OK != ucc_context_config_modify(ctx_config, NULL, "ESTIMATED_NUM_EPS", str_buf)) {
        UCC_ERROR("UCC context config modify failed for estimated_num_eps");
        goto cleanup_lib;
    }

    sprintf(str_buf, "%u", opal_process_info.num_local_peers + 1);
    if (UCC_OK != ucc_context_config_modify(ctx_config, NULL, "ESTIMATED_NUM_PPN", str_buf)) {
        UCC_ERROR("UCC context config modify failed for estimated_num_eps");
        goto cleanup_lib;
    }

    if (UCC_OK != ucc_context_create(cm->ucc_lib, &ctx_params, ctx_config, &cm->ucc_context)) {
                              ...
                              oshmem_group_t         *osh_group)
{
    ...
    for (int i = 0; i < osh_group->proc_count; i++) {
        pe_array[i] = osh_group->proc_vpids[i];
        if (osh_group->proc_vpids[i] == osh_group->my_pe) {
            index = i;
        }
    }
    ...
    if (UCC_OK != ucc_team_create_post(&cm->ucc_context, 1, &team_params, &ucc_module->ucc_team)) {
